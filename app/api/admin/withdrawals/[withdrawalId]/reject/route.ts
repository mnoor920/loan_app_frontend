...
export async function POST(
  ...
  try {
    ...
    const response = await fetch(`${API_URL}/api/admin/withdrawals/${params.withdrawalId}/reject`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      return NextResponse.json(
        { success: false, error: data.message || data.error || 'Failed to reject withdrawal' },
    ...
    console.error('Error in reject withdrawal API route:', error);
    return NextResponse.json(
