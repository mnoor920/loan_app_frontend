...
export default function LoanDetailView({ loanId }: LoanDetailViewProps) {
  ...
  // Handle approve with modal data
  const handleGenerateWalletCode = (userId: string, userName: string) => {
    setWalletCodeUserId(userId);
    setWalletCodeUserName(userName);
    setShowWalletCodeModal(true);
  };

  const handleApproveLoan = async (approvalData: {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white dark:bg-gray-900 p-6 rounded-lg border border-gray-200 dark:border-gray-800">
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="text-sm font-medium text-gray-900 dark:text-gray-100">{loan.applicationNumber}</div>
              </div>
          ...
          <div className="bg-white dark:bg-gray-900 p-6 rounded-lg border border-gray-200 dark:border-gray-800">
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2 mb-2">
                <span className={`px-2 py-0.5 text-[10px] font-bold uppercase rounded-full ${loan.applicant.isActive
                  ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400'
                  : 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400'
                  }`}>
                  {loan.applicant.isActive ? 'Active Account' : 'Disabled Account'}
                </span>
                <span className="px-2 py-0.5 text-[10px] font-bold uppercase rounded-full bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400">
                  Wallet: {formatCurrency(loan.applicant.walletBalance)}
                </span>
              </div>
            ...
            {loan.applicant.profile && (
              <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                ...
                <div className="text-sm font-medium text-gray-900 dark:text-gray-100 mb-1">
                  {loan.applicant.profile.activationStatus === 'completed' ? 'Activated' : 'Pending Activation'}
                </div>
                <div className="text-xs text-gray-500 dark:text-gray-500">
                  Step {loan.applicant.profile.currentStep}/6 completed
                </div>
                {loan.applicant.profile.completedAt && (
                  <div className="text-xs text-gray-500 dark:text-gray-500 mt-1">
                    Completed: {formatDate(loan.applicant.profile.completedAt)}
                  </div>
                )}
              </div>
            )}
        ...
        <div className="lg:col-span-2 bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-800 overflow-hidden">
          ...
          <div className="p-6">
            ...
            {activeTab === 'applicant' && (
              <div className="space-y-8">
                ...
                <section>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                    ...
                    <div>
                      ...
                      <div className="text-sm font-medium text-gray-900 dark:text-gray-100 capitalize">
                        {loan.applicant.profile?.maritalStatus || 'N/A'}
                      </div>
                    </div>
                    <div>
                      <label className="block text-xs text-gray-500 dark:text-gray-500 mb-1">Nationality</label>
                      <div className="text-sm font-medium text-gray-900 dark:text-gray-100">
                        {loan.applicant.profile?.nationality || 'N/A'}
                      </div>
                    </div>
                    <div>
                      <label className="block text-xs text-gray-500 dark:text-gray-500 mb-1">ID Type & Number</label>
                      <div className="text-sm font-medium text-gray-900 dark:text-gray-100">
                        {loan.applicant.profile?.idType && loan.applicant.profile?.idNumber
                          ? `${loan.applicant.profile.idType}: ${loan.applicant.profile.idNumber}`
                          : 'N/A'}
                      </div>
                    </div>
                ...
                <section className="pt-6 border-t border-gray-100 dark:border-gray-800">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                    <div>
                      <label className="block text-xs text-gray-500 dark:text-gray-500 mb-1">Wallet Balance</label>
                        ...
                        {loan.status === 'Approved' && (
                          <>
                            <button
                              ...
                              className="text-xs px-2 py-1 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 rounded hover:bg-blue-200 dark:hover:bg-blue-900/50 transition-colors flex items-center gap-1"
                              title="Modify wallet balance"
                            >
                              <Edit3 className="w-3 h-3" />
                              Modify
                            </button>
                            {loan.applicant.walletBalance > 0 && (
                              <button
                                onClick={() => handleGenerateWalletCode(loan.userId, `${loan.applicant.firstName} ${loan.applicant.lastName}`)}
                    ...
                    <div>
                      <label className="block text-xs text-gray-500 dark:text-gray-500 mb-1">Bank Name</label>
                      <div className="text-sm font-medium text-gray-900 dark:text-gray-100">
                        {loan.applicant.profile?.bankName || 'N/A'}
                      </div>
                    </div>
                    <div>
                      <label className="block text-xs text-gray-500 dark:text-gray-500 mb-1">Account Holder</label>
                      <div className="text-sm font-medium text-gray-900 dark:text-gray-100">
                        {loan.applicant.profile?.accountHolderName || 'N/A'}
                      </div>
                    </div>
                    <div>
                      <label className="block text-xs text-gray-500 dark:text-gray-500 mb-1">Account Number</label>
                      <div className="text-sm font-medium text-gray-900 dark:text-gray-100">
                        {loan.applicant.profile?.accountNumber || 'N/A'}
                      </div>
                    </div>
                    <div>
                      <label className="block text-xs text-gray-500 dark:text-gray-500 mb-1">Account Type</label>
                      <div className="text-sm font-medium text-gray-900 dark:text-gray-100 capitalize">
                        {loan.applicant.profile?.accountType || 'N/A'}
                      </div>
                    </div>
                  </div>
                </section>
                ...
                {/* Family Relatives */}
                {loan.applicant.profile?.familyRelatives && loan.applicant.profile.familyRelatives.length > 0 && (
                  <section className="pt-6 border-t border-gray-100 dark:border-gray-800">
                    <h3 className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-4 flex items-center gap-2">
                      <User className="w-4 h-4" />
                      Family Relatives
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      {loan.applicant.profile.familyRelatives.map((relative, idx) => (
                        <div key={idx} className="p-3 bg-gray-50 dark:bg-gray-800/50 rounded-lg border border-gray-100 dark:border-gray-800">
                          <div className="font-medium text-gray-900 dark:text-gray-100">{relative.fullName}</div>
                ...
                {/* Signature */}
                {loan.applicant.profile?.signatureData && (
                  <section className="pt-6 border-t border-gray-100 dark:border-gray-800">
                    <h3 className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-4">Signature</h3>
                    <div className="bg-white dark:bg-gray-200 p-4 rounded-lg border border-gray-200 inline-block">
                      <img
                        src={loan.applicant.profile.signatureData}
            ...
            {activeTab === 'documents' && (
              <div className="space-y-4">
                {loan.applicant.documents && loan.applicant.documents.length > 0 ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {loan.applicant.documents.map((doc) => (
                      <div key={doc.id} className="p-4 border border-gray-200 dark:border-gray-700 rounded-lg bg-gray-50 dark:bg-gray-800/50 flex flex-col justify-between">
                        <div>
                          <div className="flex justify-between items-start mb-2">
                            <div className="text-sm font-bold text-gray-900 dark:text-gray-100 uppercase tracking-tight">
                              {doc.documentType.replace('_', ' ')}
                            </div>
                            <span className={`px-2 py-0.5 text-[10px] font-bold uppercase rounded-full ${doc.verificationStatus === 'verified'
                              ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400'
      ...
      {/* Wallet Withdrawal Code Modal */}
      {showWalletCodeModal && walletCodeUserId && (
        <GenerateCodeModal
          isOpen={showWalletCodeModal}
          onClose={() => {
            setShowWalletCodeModal(false);
            setWalletCodeUserId(null);
            setWalletCodeUserName('');
          }}
          userId={walletCodeUserId}
          applicantName={walletCodeUserName}
          codeType="wallet"
        />
      )}

      {/* Wallet Modification Modal */}
      {loan && loan.status === 'Approved' && (
        <WalletModificationModal
          ...
          userId={loan.userId}
          currentWalletBalance={loan.applicant.walletBalance}
          currentLoanAmount={loan.loanAmount}
          currentMonthlyPayment={loan.monthlyPayment}
          currentInterestRate={loan.interestRate}
          currentDurationMonths={loan.durationMonths}
          onSuccess={(updatedData) => {
            // Update loan state with new data
            setLoan(prev => prev ? {
              ...prev,
              loanAmount: updatedData.loanAmount || prev.loanAmount,
              monthlyPayment: updatedData.monthlyPayment || prev.monthlyPayment,
              totalAmount: updatedData.totalAmount || prev.totalAmount,
              applicant: {
                ...prev.applicant,
            ...
            setFormData(prev => ({
              ...prev,
              loanAmount: (updatedData.loanAmount || loan.loanAmount).toString(),
              durationMonths: loan.durationMonths.toString(),
              interestRate: loan.interestRate.toString(),
            }));
      ...
      {/* Approve Loan Modal */}
      {loan && loan.status === 'Pending Approval' && (
        <ApproveLoanModal
